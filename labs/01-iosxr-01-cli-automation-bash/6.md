...
### Configuration Files that will applied by Ansible  

The configuration file per router is located in the two folders `r1` and `r2` under `ansible/` folder:  
...
<p style="margin: 2em 0!important;padding: 1em;font-family: CiscoSans,Arial,Helvetica,sans-serif;font-size: 1em !important;text-indent: initial;background-color: #e6f2f7;border-radius: 5px;box-shadow: 0 1px 1px rgba(0,127,171,0.25);">set export ANSIBLE_HOST_KEY_CHECKING=false to avoid having to add the router key to known_hosts for this run.</p>
