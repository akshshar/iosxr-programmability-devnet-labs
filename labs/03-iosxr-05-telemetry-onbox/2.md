...
In this lab, we intend to enable IPv6 on b2b interfaces on the two routers (`r1` and `r2`) and stream IPv6 neighbor information to an onbox client over gRPC.   

We will build a docker container with a  `dial-in` Telemetry collector/client wherein the client initiates a connection to the router locally and then subscribes to a pre-configured subscription on the router.    
