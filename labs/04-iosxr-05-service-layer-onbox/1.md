# Before we Begin
Service-Layer APIs offer high-performance access to the network infrastructure layer components of IOS-XR such as RIB, LSD (Label switch Database), Interface events and BFD events. In this lab, we intend to  construct our first onbox service-layer API client that runs inside a container on the router, interacts over gRPC locally, and configures routes into the IOS-XR RIB. This lab will take you through all the steps necessary to get you up and running - from installation of gRPC inside the container image to deploying the container on the router and programming route objects into the router's RIB.

## Objectives

* Reserve and Familiarize yourself with the IOS-XR Devnet Sandbox
* Configure and Enable Service-layer on the router
* Understand the Application Hosting infrastructure on IOS-XR
* Build your Container (Docker) image with dependencies and client pre-installed
* Spin up the Container on the Router and program routes into IOS-XR RIB.
