# Before we Begin
Service-Layer APIs offer high-performance access to the network infrastructure layer components of IOS-XR such as RIB, LSD (Label switch Database), Interface events and BFD events. In this lab, we intend to write our first python service-layer API client that interacts over gRPC and configures routes into the IOS-XR RIB. This lab will take you through all the steps necessary to get you up and running - from installation of gRPC on the development environment to programming route objects into the router's RIB.

## Objectives

* Reserve and Familiarize yourself with the IOS-XR Devnet Sandbox
* Configure and Enable Service-layer on the router
* Install gRPC and regenerate bindings from proto files
* Client Code: Setting up an initialization channel
* Client Code: Registering against a vrf
* Client Code: Programming Route objects into IOS-XR RIB
