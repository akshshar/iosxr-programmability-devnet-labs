# Before we Begin
Service-Layer APIs offer high-performance access to the network infrastructure layer components of IOS-XR such as RIB, LSD (Label switch Database), Interface events and BFD events. In this lab, we intend to write our first c++ service-layer API client that interacts over gRPC and configures routes into the IOS-XR RIB. This lab will take you through all the steps necessary to get you up and running - from installation of gRPC on the development environment to programming route objects into the router's RIB.
