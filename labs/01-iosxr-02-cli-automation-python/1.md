# Before we Begin
If you've been busy developing or utilizing external tools that help automate the IOS-XR CLI over Telnet/SSH using expect style send/receive transactions, then it is time to take a look at the new ZTP hooks embedded on the device. In this lab, we look at ZTP **Python** hooks, enabling you to deterministically handle IOS-XR CLI automation across show commands, config-apply, config-replace and more, using on-box python scripts executed locally or remotely over SSH.

## Objectives

* Reserve and Familiarize yourself with the IOS-XR Devnet Sandbox
* Understand the ZTP infrastucture in IOS-XR
* Familiarize yourself with the ZTP python helper library
* Write your first onbox IOS-XR CLI python script
* Automate IOS-XR CLI using python script executed over SSH
* Extend onbox python script execution to Config Management tools like Ansible, puppet etc.
